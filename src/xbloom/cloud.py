...
truth for recipes; when they don't, the caller stays fully local and
BLE-only.
...
def language_type_for(code: str | None) -> int:
    """Map a language code (BCP-47) to xBloom's
...
# per-client (from the caller's locale) via ``_envelope``.
_BASE_ENVELOPE = {
...
# RSA public key the xBloom clients use (1024-bit). Bodies of tu* calls are
# encrypted with it.
_RSA_PUBLIC_KEY_B64 = (
...
class XBloomCloudClient:
    ...
    async def list_recipes(self, member_id: int, token: str) -> list[Recipe]:
        ...
        back to its origin. Callers typically merge them into one library, tagging shared
...
class XBloomCloudSession:
    ...
    This is the portable seam a host layer talks to. It owns the credential
    lifecycle — including automatic re-login when the token expires (error
    ``10001``) — so every consumer gets identical behavior without
    reimplementing it.

    The caller's only jobs are to *supply* the credentials and to
    *persist* a refreshed token. Persistence is delegated back via the optional
    ``on_token_refreshed(member_id, token)`` callback — this library never
    touches config stores, files, or any host-specific storage.
    ...
    Args:
        ...
        password: account password, or ``None`` if the user chose not to store
            ...
            session re-raises the expiry so the caller can prompt a
            re-login instead.
