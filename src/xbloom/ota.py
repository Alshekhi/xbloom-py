...
Transport — a YMODEM-flavoured protocol over the machine's **dedicated** OTA GATT
...
caller must hold an exclusive, stable connection (no other BLE activity to the
machine) and
...
# -- GATT (the machine's OTA service) ---------------------------------------------- #
...
class XBloomOtaFlasher:
    """Drive a firmware flash over BLE against the machine's OTA service.
        ...
        *,
        ...
        Args:
            chunk_delay: seconds to pause between 16-byte writes. The right value
                ...
                caller supplies it rather than this library hard-coding it.
