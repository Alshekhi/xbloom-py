"""Prove a host adapter's spec-derived values equal the pre-refactor literals.

A host's config UI cannot be imported here, so this replicates the *pure*
derivations it performs against the spec — ratio options, the cup dose table,
and each numeric control's config — and checks them against a frozen snapshot
of the hand-written values. The one intended change is pour temperature's
floor (40, not the old UI's 20).
"""
